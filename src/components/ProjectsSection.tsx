...
const useTilt = (ref: React.RefObject<HTMLDivElement>) => {
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Adjust divisor for tilt intensity
...
const ProjectsSection = () => {
  ...
  const projects = [
    {
      ...
      description:
        "Personalised book recommendation engine using web scraping, large language models and a retrieval-augmented generation (RAG) strategy. An end-to-end recommendation engine that scrapes Goodreads, processes data and uses LLMs to deliver tailored reading lists.",
    ...
    },
    {
      ...
      description:
        "Analyses Twitter data with Hugging Face transformers to infer sentiments and deliver highly targeted ads. Analyses user sentiments and interests from tweets to serve personalised advertisements and content recommendations.",
    ...
    },
    {
      ...
      description:
        "Computer-vision app providing real-time audio descriptions of surroundings for visually impaired users. An assistive tool that combines object recognition and scene understanding to enhance navigation for those with low vision.",
    ...
    },
    {
      ...
      description:
        "Predictive modelling to optimise healthcare costs and improve outcomes using complex Medicare datasets. Data-driven recommendations that enhance financial planning and resource allocation for more efficient healthcare.",
  ...
  useEffect(() => {
    ...
    gsap.set(gridRef.current?.children, { opacity: 0, y: 80, scale: 0.8, filter: 'blur(10px)' });

    // Animate title
    tl.to(titleRef.current, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 1, ease: 'power2.out' })
    ...
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div ref={titleRef} className="text-center space-y-4 mb-16">
          ...
          <div className="w-16 h-0.5 bg-primary/60 rounded-full mx-auto" />
        </div>
        ...
        <div ref={gridRef} className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => {
            ...
            return (
              ...
              >
                {/* Project Image */}
                <div className="relative h-56 overflow-hidden rounded-t-lg bg-background-secondary">
                  <img
                    src={project.image}
                    alt={project.title}
                ...
                <div className="p-6 space-y-4">
                  ...
                  <p className="text-muted-foreground font-inter text-sm leading-relaxed group-hover:text-foreground transition-colors duration-300">
                    {project.description}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 text-xs font-inter bg-muted text-muted-foreground rounded-full border border-border group-hover:border-primary/30 group-hover:text-foreground transition-colors duration-300"
                      >
                        {tag}
                      </span>
                    ))}
